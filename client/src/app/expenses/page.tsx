    ...
    const aggregatedData: AggredatedDataItem[] = useMemo(() => {
        const filtered: AggregatedData = expenses.filter(
            (data: ExpenseByCategorySummary) => {
                ...
                const dataDate = parseDate(data.date); // Formatting the date from backend just before matching the dates
            ...
            if (!acc[data.category]) {
                ...
                acc[data.category].color = `#${Math.floor(Math.random() * 16777215).toString(16)}`; // Get a random color for new category
